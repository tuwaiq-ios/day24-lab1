...
class ViewController: UIViewController {
    ...
    @objc func AddNewUser(){
        Firestore.firestore().document("users/NameUser").setData(["age": 23, "city": "Dammam", "id": "87", "name": "Lama"])
    }
    
    
    
    @objc func deleteUser(){
        Firestore.firestore().document("users/NameUser").delete()
        self.tlabel.text = " "
    }
    ...
    @objc func show(){
        Firestore.firestore().document("users/NameUser").addSnapshotListener{ document, error in
            if error != nil {
                print(error)
                return
            }
