...
//  Day24
//
//  Created by HANAN on 21/03/1443 AH.
//
import UIKit
import FirebaseAuth
...
class ViewController: UIViewController {
    ...
    let buttonadd : UIButton = {
        ...
        $0.addTarget(self, action: #selector(showBtn), for: .touchUpInside)
    ...
    @objc func addButton() {
            Firestore.firestore().document("users/first").setData([
                "id": 2,
                "name": "Hanan"
    ...
    @objc func showBtn(){
            ...
            Firestore.firestore().collection("users").whereField("name", isEqualTo: "Hanan").addSnapshotListener { snapshot, error in
                if error != nil {
                    print(error as Any)
                    return
                }
//                 to show data in Lable
        ...
        @objc func delButton() {
            Firestore.firestore().document("users/first").delete()
